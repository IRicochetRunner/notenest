  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        let results = [];
        try {
          const r1 = await fetch(`/api/itunes?term=${encodeURIComponent(search)}&limit=8`);
          if (r1.ok) { const d = await r1.json(); results = d.results || []; }
        } catch(e) {}
        if (!results.length) {
          const r2 = await fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(search)}&entity=song&limit=8&media=music`);
...
// ── MAIN EXPORT ───────────────────────────────────────────────
export default function SetlistBuilder({ songs, dark, isPro = false }) {
  ...
  const saveGig = (gig) => {
    if (gigs.find(g => g.id === gig.id)) {
      ...
      // Free users limited to 1 setlist
      if (!isPro && gigs.length >= 3) { setShowProGate(true); setShowGigModal(false); return; }
  ...
  if (view === "gigs") return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={() => {
            if (!isPro && gigs.length >= 3) { setShowProGate(true); return; }
            setEditingGig(null); setShowGigModal(true);
          }}
          className="flex items-center gap-2 bg-[#1a3a8f] text-white font-black px-5 py-2.5 rounded-xl border-none cursor-pointer shadow-[0_4px_0_#0f2460] hover:-translate-y-0.5 transition-all text-sm"
          style={{fontFamily:"Nunito, sans-serif"}}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0d1b3e]/60 backdrop-blur-md p-4"
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-8 text-center">
              <div className="w-14 h-14 bg-amber-50 border-2 border-amber-200 rounded-3xl flex items-center justify-center mx-auto mb-4">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-amber-500"><path d="M12 1C8.676 1 6 3.676 6 7v1H4v15h16V8h-2V7c0-3.324-2.676-6-6-6zm0 2c2.276 0 4 1.724 4 4v1H8V7c0-2.276 1.724-4 4-4zm0 9a2 2 0 110 4 2 2 0 010-4z"/></svg>
              </div>
              <div className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200 text-amber-600 text-xs font-black px-3 py-1 rounded-full mb-3">PRO FEATURE</div>
              <h3 className="font-black text-xl text-[#0d1b3e] mb-2" style={{fontFamily:"Nunito,sans-serif"}}>Unlimited Setlists</h3>
              <p className="text-sm text-[#6b7a9e] mb-6">Free plan includes 3 gigs. Upgrade to Pro to create unlimited gigs and setlists.</p>
              <div className="bg-[#f0f4ff] rounded-2xl p-4 mb-6 text-left">
                {["Unlimited gigs & setlists","Full activity history","Skills breakdown","Song structure diagram","All 18 packs"].map(f => (
              ...
              <button className="w-full bg-[#1a3a8f] text-white font-black py-4 rounded-2xl shadow-[0_4px_0_#0f2460] hover:-translate-y-0.5 transition-all border-none cursor-pointer text-sm mb-2" style={{fontFamily:"Nunito,sans-serif"}}>
                Upgrade to Pro — $7/mo
              </button>
              <button onClick={() => setShowProGate(false)} className="text-xs text-[#6b7a9e] bg-transparent border-none cursor-pointer hover:text-[#0d1b3e]">Maybe later</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0d1b3e]/60 backdrop-blur-md p-4"
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-8 text-center">
              ...
              <div className="inline-flex items-center gap-1 bg-amber-50 border border-amber-200 text-amber-600 text-xs font-black px-3 py-1 rounded-full mb-3">PRO FEATURE</div>
              <h3 className="font-black text-xl text-[#0d1b3e] mb-2" style={{fontFamily:"Nunito,sans-serif"}}>Unlimited Setlists</h3>
              <p className="text-sm text-[#6b7a9e] mb-6">Free plan includes 1 setlist. Upgrade to Pro to create unlimited gigs and setlists.</p>
              <div className="bg-[#f0f4ff] rounded-2xl p-4 mb-6 text-left">
                {["Unlimited gigs & setlists","Full activity history","Skills breakdown","Song structure diagram","All 18 packs"].map(f => (
                  <div key={f} className="flex items-center gap-2.5 py-1.5">
                    <div className="w-4 h-4 bg-[#1a3a8f] rounded-full flex items-center justify-center flex-shrink-0">
